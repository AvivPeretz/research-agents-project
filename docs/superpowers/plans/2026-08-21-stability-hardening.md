...
Cerebras was evaluated as a 4th waterfall fallback after this plan was written, integrated (`config.py`, `agents/base_agent.py`), and live-tested end-to-end. It was then **fully removed**: real calls returned a `402 payment_required_error` on every model available to the account, meaning Cerebras's free tier requires billing/credit setup despite the operator's own dashboard advertising usable free-tier quotas — this violates the hard requirement that every fallback provider be usable with zero payment setup. This is a permanent decision; do not re-propose Cerebras without new evidence the billing requirement has changed.

### Post-implementation update (2026-08-23): Gemma 4 added

Gemma 4 was evaluated as a new fallback tier and added to the waterfall, live-tested end-to-end (`config.py`, `agents/base_agent.py`). Unlike Cerebras, it required **no new API key or scope**: both candidate models (`models/gemma-4-26b-a4b-it` and `models/gemma-4-31b-it`) are served through the exact same `genai.Client`/`GEMINI_API_KEY` credential already used for the Gemini fallback, confirmed via a live `models.list()` call and real `generate_content` calls to both. `gemma-4-26b-a4b-it` (MoE) was chosen as the default — same input/output token limits as the 31b variant per `models.list()`, but MoE gives near-31B quality at a fraction of the compute, which matters at a fallback tier that's already past two higher-priority providers. Live-verified this session: a real `generate_content` call succeeded, structured JSON output via `response_mime_type="application/json"` produced output that `domain.schemas.LiteratureReport.model_validate_json()` parsed without error, and a ~15,000-char manuscript-excerpt-shaped payload was accepted without a context-length error. Rate limits were **not** confirmed with an authoritative number — the real response exposed no rate-limit HTTP headers, and Google's official docs (`ai.google.dev/gemini-api/docs/rate-limits`, `ai.google.dev/gemma/docs/core`, both checked live 2026-08-23) don't list Gemma limits at all; the only numbers found were from an unofficial community forum post, not cited as fact. Gemma 4 reuses the identical Gemini code path in `_ask_provider()` (same client, same `.text` response shape) — no separate code branch was needed beyond routing on model id. The final waterfall is Groq → Gemini → Gemma 4 → NVIDIA NIM → OpenAI.
